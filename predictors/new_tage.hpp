...
template<u64 LOGLB=8, u64 NUMG=13, u64 LOGG=11, u64 LOGB=8, u64 TAGW=20, u64 GHIST=234, u64 LOGP1=12, u64 GHIST1=6>
struct new_tage : predictor {
    ...
    val<1> predict2(val<64> inst_pc)
    {
        ...
        for (u64 i=0; i<NUMG; i++) {
            gindex[i] = lineaddr ^ gfolds.template get<0>(i) ^ (gfolds.template get<1>(i) << 1);
        }
        gindex.fanout(hard<4>{});

        // compute hashed tags
        for (u64 i=0; i<NUMG; i++) {
            htag[i] = val<HTAGBITS>{lineaddr ^ (lineaddr >> HTAGBITS)} ^ gfolds.template get<1>(i) ^ (gfolds.template get<0>(i) >> 1);
        }
        htag.fanout(hard<2>{});
