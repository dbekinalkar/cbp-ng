 ...
 * GHR_LEN: length of the global history register
 * 0 = global, 1 = local predictor
*/

template <u64 BHT_SIZE_BITS=6, u64 PHT_SIZE_BITS=12, u64 HIST_LEN=6, u64 GHR_LEN=6>
struct tournament_predictor : predictor {
    ...
    // global predictor
    ram<val<2>, BHT_SIZE> g_bht;
    reg<2> g_bh;
    reg<GHR_LEN> g_ghr;

    // choice predictor
    ram<val<2>, BHT_SIZE> t_bht;
    reg<2> t_bh;
    reg<2> chosen_bh;
    ...
    inline val<GHR_LEN> update_ghr(val<1> taken) {
        val<GHR_LEN> shifted = g_ghr << hard<1>{};
