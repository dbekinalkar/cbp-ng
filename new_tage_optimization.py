...
def objective(trial):
    params = [
        trial.suggest_int("LOGLB", 4, 10),
        trial.suggest_int("NUMG", 4, 16),
        trial.suggest_int("LOGG", 9, 14),
        trial.suggest_int("LOGB", 4, 14),
        trial.suggest_int("TAGW", 7, 21),
        trial.suggest_int("GHIST", 160, 240),
        trial.suggest_int("LOGP1", 11, 16),
    ...
    res = subprocess.run(f'./compile_custom {trial.number} -DPREDICTOR="new_tage<{p}>"', shell=True)
    try:
        res.check_returncode()
    except subprocess.CalledProcessError:
        return 1e9
    subprocess.run(f'./run_all ./cbp_{trial.number} ./traces {outdir} > /dev/null 2>&1', shell=True)
...
study = optuna.create_study(study_name="new_tage_optimization", storage="sqlite:///new_tage.db", load_if_exists=True)
